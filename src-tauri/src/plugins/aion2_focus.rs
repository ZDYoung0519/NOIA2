...
const FOLLOW_FOCUS_WINDOW_LABELS: [&str; 3] =
    ["dps-overlay", "dps-overlay-pvp", "dps-overlay-buff"];
...
mod windows_impl {
    ...
    pub fn start<R: Runtime>(app: AppHandle<R>) {
        ...
        std::thread::spawn(move || {
            ...
            while let Ok(hwnd_raw) = rx.recv() {
                ...
                let (dps_manual_hidden, auto_hide_enabled, dps_always_on_top) = app_for_processor
                    .try_state::<Aion2FocusState>()
                    .map(|state| {
                        (
                            state.dps_manual_hidden.load(Ordering::Relaxed),
                            state.auto_hide_enabled.load(Ordering::Relaxed),
                            state.dps_always_on_top.load(Ordering::Relaxed),
                        )
                    })
                    .unwrap_or((false, true, false));

                if !auto_hide_enabled || dps_always_on_top {
                    continue;
                }
    ...
    fn is_overlay_related_window_focused<R: Runtime>(app: &AppHandle<R>) -> bool {
        ["dps_settings", "dps_log", "dps_detail_v2"]
            .iter()
            .any(|label| {
