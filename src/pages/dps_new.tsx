  ...
  const schedulePaint = () => {
    ...
    requestAnimationFrame(() => {
      ...
      stats.slice(0, 8).forEach((p, i) => {
        ...
        r.bar.style.transform = `scaleX(${dmg / maxDmg})`;
        const playerColor =
          mainId != null && p.actorId === mainId
            ? dpsAppearanceSetting.mainPlayerColor
            : dpsAppearanceSetting.otherPlayerColor;
        r.bar.style.background = isHunterSkin
          ? `linear-gradient(90deg, ${colorToRgba(playerColor, 0.22)} 0%, ${colorToRgba(playerColor, 0.12)} 78%, transparent 100%)`
          : playerColor;
        r.bar.style.opacity = isHunterSkin ? "1" : "";
        if (r.accentBar) {
          r.accentBar.style.transform = `scaleX(${dmg / maxDmg})`;
          r.accentBar.style.background = `linear-gradient(90deg, transparent 0%, ${colorToRgba(playerColor, 1)} 85%, ${colorToRgba(playerColor, 0.9)} 100%)`;
        }
        ...
        setText(r.name, p.actorName || `Player ${p.actorId}`);
        if (p.actorServerId) {
          let name = serverNameCache.current.get(p.actorServerId);
          if (!name) {
            name = getServerShortName(Number(p.actorServerId));
            serverNameCache.current.set(p.actorServerId, name);
          }
          setText(r.server, `[${name}]`);
        } else {
          setText(r.server, "");
        }
        setText(r.dps, fmtDps(p.dps));
        setText(r.dmg, fmtDmg(dmg));
        setText(r.pct, `${(p.damageShare * 100).toFixed(1)}%`);
      });
      for (let i = stats.length; i < 8; i++) rowsRef.current[i].root.style.display = "none";
    });
  };
