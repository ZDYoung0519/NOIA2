...
const getStatDamage = (stats?: SkillStats | null) => Number(stats?.total_damage ?? 0);
const getStatMinDamage = (stats?: SkillStats | null) => Number(stats?.min_damage ?? 0);
const getStatMaxDamage = (stats?: SkillStats | null) => Number(stats?.max_damage ?? 0);
const getSpecialCount = (stats: SkillStats | null | undefined, key: string) =>
  Number(stats?.special_counts?.[key] ?? 0);
...
export function DpsDetailContent({ payload }: { payload: DpsDetailPayload | null }) {
  ...
  return (
    <div className="flex w-full flex-col gap-2 text-slate-100">
      <section className="rounded-lg bg-transparent p-0">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2">
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
                ...
                <div className="mt-1 text-xs font-semibold text-slate-100">{summary.totalHits}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-6 gap-2">
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
                ...
                <div className="mt-1 text-xs font-semibold text-slate-300">{summary.parryRate}</div>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
                <div className="text-xs text-slate-400 uppercase">{t("dps.detail.multi")}</div>
                <div className="mt-1 text-xs font-semibold text-rose-200">{summary.multiRate}</div>
              </div>
            </div>
          </div>
        ) : (
      ...
      <section className="overflow-hidden rounded-lg border border-white/10 bg-transparent">
        {skillRows.length > 0 ? (
          <div className="overflow-x-auto overflow-y-visible [scrollbar-color:rgba(148,163,184,0.35)_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-400/25 [&::-webkit-scrollbar-track]:bg-transparent">
            <div className="min-w-[1000px]">
              ...
              {skillRows.map((row) => {
                const critRate =
                  row.counts > 0 ? (getSpecialCount(row.stats, "CRITICAL") / row.counts) * 100 : 0;
                const backRate =
                ...
                const multiRate =
                  row.counts > 0 ? (getSpecialCount(row.stats, "MULTIHIT") / row.counts) * 100 : 0;
                const multiHitDamage = getSpecialCount(row.stats, "MULTIHITDMG");
                const averageDamage = row.counts > 0 ? Math.floor(row.totalDamage / row.counts) : 0;
                const totalPercent =
