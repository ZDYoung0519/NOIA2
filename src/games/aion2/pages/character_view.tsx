...
import { fetchFengwoV2, formatFengwoResponse } from "@/games/aion2/lib/fetchFengwo";
import { getServerShortName } from "@/games/aion2/lib/servers";

import { ActorNameCell, getActorClassName } from "@/games/aion2/components/dps-table-cells";

import { renderEquipmentInfo } from "@/games/aion2/components/aion2_ui/slot-equip";
import { renderArcanaSlot } from "@/games/aion2/components/aion2_ui/slot-arcana";
import { renderSkillSlot } from "@/games/aion2/components/aion2_ui/slot-skill";
import {
  renderStatInfo,
  renderDetailedStatInfo,
  renderStat,
  FormulaStat,
} from "@/games/aion2/components/aion2_ui/stat-info";
import { getStatEntriesMap, StatEntry } from "@/games/aion2/lib/stat-utils";
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 rounded-full bg-white/8 px-3 py-1.5">
            <img src="/aion2/profile_level_icon_pc.png" alt="Item level" className="h-5 w-4" />
            <span className="text-sm font-semibold text-white">{itemLevel ?? "--"}</span>
          </div>

          <div className="flex items-center gap-2 rounded-full bg-white/8 px-3 py-1.5">
            <img src="/aion2/profile_power_icon_pc.png" alt="Combat power" className="h-5 w-5" />
          ...
          <div className="flex items-center gap-2 rounded-full bg-white/8 px-3 py-1.5">
            <img src="/aion2/fengwo.png" alt="Fengwo score" className="h-5 w-5" />
            <span className="text-sm font-semibold text-white">
...
function StatDetailPage({ statEntriesMap }: { statEntriesMap: Record<string, StatEntry[]> }) {
  ...
  const { tAion2Stats } = useAppTranslation();
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="bg-background/60 grid grid-cols-1 justify-center gap-10 rounded-lg p-4 backdrop-blur-sm sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
        ...
        <div className="space-y-3">
          {renderStat(Defense, "Defense", "", 0, tAion2Stats)}
          {renderStat(Evasion, "Evasion", "", 0, tAion2Stats)}
          {renderStat(CriticalResist, "CriticalResist", "", 0, tAion2Stats)}
          {renderStat(DecreaseDamage, "DecreaseDamage", "", 0, tAion2Stats)}
          {renderStat(DecreaseCriticalDamage, "DecreaseBackAttack", "", 0, tAion2Stats)}
          {renderStat(DecreaseBackAttack, "DecreaseBackAttack", "", 0, tAion2Stats)}
          {renderStat(PerfectResist, "PerfectResist", "", 0, tAion2Stats)}
          {renderStat(HardHitResist, "HardHitResist", "", 0, tAion2Stats)}
          {renderStat(AdditionalHitResistRate, "AdditionalHitResistRate", "", 0, tAion2Stats)}
        </div>
      </Card>
    </div>
  );
